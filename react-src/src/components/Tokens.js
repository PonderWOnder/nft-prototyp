...
class Tokens extends Component {
  ...
  render() {
    return (
      <div className='m-2'>
        {this.props.MyTokens.length>0 || this.props.showCircles ?
            ...
            {this.props.MyTokens.map((token,index) =>
              <div key={index}>
                ...
                {(token[6] > Math.floor(new Date().getTime()/1000) || token[6]==='0') ? //need to do some type conversion to make this rock solid!
                  <button className='rounded-lg hover:bg-indigo-400 hover:text-white px-2' onClick={e => this.props.resttoken(e, token[1]+token[2],token[0])}> <span>  Connect </span> </button>:
